...
async function main() {
  ...
  const { models, plans, presets } = await import("../src/db/schema");
  const { MODELS, PLANS, PRESETS } = await import("../src/db/catalog");
  ...
  console.log(`seeded ${PLANS.length} plans, ${MODELS.length} models, ${PRESETS.length} presets`);
  await pool.end();
}
