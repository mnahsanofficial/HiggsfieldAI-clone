...
PREV=$(npx --yes vercel@latest inspect "$(echo "$S" | awk '{print $2}')" 2>&1 | grep -o 'https://docket-nahsan-[a-z0-9]*-mnahsanofficials-projects\.vercel\.app' | head -1)
...
for i in $(seq 1 40); do curl -s https://docket-nahsan.vercel.app/api/health | grep -q "\"commit\":\"$SHA\"" && { echo "PRODUCTION GREEN: /api/health reports $SHA"; exit 0; }; sleep 10; done
