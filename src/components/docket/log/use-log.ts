...
export function useLog(initial: LogEntry[], initialBalance: number, opts: { credits?: CreditEvent[]; more?: boolean } = {}) {
  ...
  const refresh = useCallback(async () => {
    ...
    const data = (await res.json()) as { entries: LogEntry[]; credits: CreditEvent[]; balanceTenths: number | null; more?: boolean };
  ...
  return { entries, credits, more, loadOlder, loadingOlder, balanceTenths, setBalance, refresh };
}
