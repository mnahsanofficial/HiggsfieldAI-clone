...
export const fixtureProvider: ImageProvider = {
  key: "test-fixture",
  async generate() {
    // Roughly the real provider's latency, so tests see runs in progress the way users do.
    await new Promise((r) => setTimeout(r, Number(process.env.FIXTURE_LATENCY_MS ?? 1500)));
