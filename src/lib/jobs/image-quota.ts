...
import { imagesPerDayFor, imagesPerNetworkPerDay } from "@/lib/billing/limits";
...
//  - per visitor: the account's plan's images_per_day (lib/billing/limits.ts), so no one uses up
//    the day for everyone; a paid plan raises it.
//  - per network: the highest plan cap per IP, so cycling guest sessions can't get around the
//    per-visitor cap, while a few reviewers behind one office IP can still each make theirs.
...
export type ImageQuota = {
  siteCapacity: number;
  siteLeft: number;
  perVisitor: number; // this visitor's plan's daily cap
  perNetwork: number;
...
export async function imageQuota(userId: string | null, ipHash: string | null, exec: Db | Tx = db): Promise<ImageQuota> {
  ...
  const [perVisitor, perNetwork] = await Promise.all([imagesPerDayFor(userId, exec), imagesPerNetworkPerDay(exec)]);
  ...
  const yoursLeft = Math.max(0, Math.min(perVisitor - mine, perNetwork - network));
  const testMode = fixtureMode();
  return { siteCapacity: IMAGE_CALLS_PER_DAY, siteLeft: testMode ? IMAGE_CALLS_PER_DAY : siteLeft, perVisitor, perNetwork, yoursLeft, resetsIn: untilUtcMidnight(), testMode };
}
...
export async function assertImageAllowance(tx: Tx, userId: string, ipHash: string | null, batchSize: number, opts: { site: boolean } = { site: true }): Promise<void> {
  ...
  if (q.yoursLeft < batchSize) {
    const byNetwork = q.yoursLeft < q.perVisitor - (await imagesToday(tx, eq(generationJobs.userId, userId)));
    if (q.yoursLeft === 0) {
      throw new DailyLimitError(
        byNetwork ? "network" : "visitor",
        byNetwork
          ? `This network has made its ${q.perNetwork} images for today. More at 00:00 UTC, in ${q.resetsIn}. Nothing was charged.`
          : `You've made your ${q.perVisitor} images for today. More at 00:00 UTC, in ${q.resetsIn}. Each account has a daily limit, so no one uses up the day for everyone${q.perVisitor < q.perNetwork ? `; a paid plan raises yours to up to ${q.perNetwork}` : ""}. Nothing was charged.`,
      );
    }
